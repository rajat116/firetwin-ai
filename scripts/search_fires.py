...
def search_2020_california_fires():
    ...
    try:
        ...
        ca_2020 = [
            ...
            and p.incident_name
            and any(keyword in p.incident_name.lower() for keyword in ["creek", "california", "ca"])
        ]

        print(f"\nFound {len(ca_2020)} California 2020 fires with 'creek' or 'ca':")
        for p in ca_2020[:10]:  # Show first 10
            print(
                f"  - {p.incident_name} ({p.fire_discovery_datetime.date()}): {p.gis_acres:,.0f} acres"
            )
    ...
    try:
        ...
        ca_fires = [
            f
            for f in fires_2020
            if f.fire_name and ("creek" in f.fire_name.lower() or "ca" in f.fire_name.lower())
        ]
