...
def test_creek_fire_2020():
    ...
    if "firms" in raw_data and len(raw_data["firms"]) > 0:
        ...
        if all(miny <= lat <= maxy for lat in lats) and all(minx <= lon <= maxx for lon in lons):
            ...
            print("  ✗ Some detections outside bbox!")
            failed += 1

        # Check dates
        dates_in_range = all(config.start_date <= d.acq_date <= config.end_date for d in detections)
        if dates_in_range:
            print("  ✓ All detections within date range")
            passed += 1
        else:
            print("  ✗ Some detections outside date range!")
            failed += 1
    ...
    if "nifc" in raw_data and len(raw_data["nifc"]) > 0:
        ...
        if any("creek" in p.incident_name.lower() for p in perimeters):
            print("  ✓ Fire name matches search")
            passed += 1
        else:
            print("  ⚠ Fire name mismatch - check if this is correct")
    ...
    if "mtbs" in raw_data and len(raw_data["mtbs"]) > 0:
        ...
        if any(f.fire_year == 2020 for f in fires):
            print("  ✓ Fire year matches (2020)")
            passed += 1
        else:
            print("  ⚠ Fire year mismatch")

        # Show sample
        if fires:
            print("  Sample MTBS record:")
            sample = fires[0]
